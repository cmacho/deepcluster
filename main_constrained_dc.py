...
def parse_args():
    ...
    parser.add_argument('--num_centers_from_labeled_tasks', type=int, default=10,
...
def main(args):
    ...
    labeled_tasks = np.load("labeled_tasks.npy")
    num_tasks, n_way, num_samples_per_class, height, width, channels = labeled_tasks.shape

    # transpose to channel first ordering
    labeled_tasks = np.transpose(labeled_tasks, (0,1,2,5,3,4))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        features, features_labeled_tasks = compute_features(dataloader, dataloader_labeled_tasks, model,
                                                   len(dataset), num_tasks * n_way * num_samples_per_class)

        print("type and shape of features_unlabeled_data and features_labeled_data:")
        print(type(features))
        print(features.shape)
        print(type(features_labeled_tasks))
        print(features_labeled_tasks.shape)

        # reshape features_labeled_tasks
        features_labeled_tasks = features_labeled_tasks.reshape((num_tasks, n_way, num_samples_per_class, -1))

        # cluster the features
        if args.verbose:
            print('Cluster the features')
        clustering_loss = deepcluster.cluster(features, features_labeled_tasks, verbose=args.verbose)

        # assign pseudo-labels
        if args.verbose:
            print('Assign pseudo labels')
        train_dataset = clustering.cluster_assign(deepcluster.images_lists,
                                                  dataset.imgs, torch.Tensor(labeled_tasks_flat))
...
def compute_features(dataloader, dataloader_labeled_tasks, model, N, num_labeled_data):
    if args.verbose:
    ...
    return features, features_labeled_tasks
